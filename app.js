import { h, render, useState, useEffect, useRef, useCallback } from "https://esm.sh/preact@10/compat";

const BAT = 16, RANGE = 133;
...
const SEASONS = [
  { id: "invierno",  label: "❄️ Invierno",  color: "#60a5fa", months: [12,1,2],  rangeEst: "75–95 km",   tip: "Frío reduce batería ~20-30%. Precalienta enchufado." },
  { id: "primavera", label: "🌸 Primavera", color: "#34d399", months: [3,4,5],   rangeEst: "100–115 km", tip: "Temperatura ideal. Mejor rendimiento del año." },
  { id: "verano",    label: "☀️ Verano",    color: "#fbbf24", months: [6,7,8],   rangeEst: "105–120 km", tip: "Calor extremo puede degradar batería. Evita carga al sol." },
  { id: "otono",     label: "🍂 Otoño",     color: "#f97316", months: [9,10,11], rangeEst: "95–110 km",  tip: "Temperaturas en descenso reducen la autonomía progresivamente." },
];
const SCOL = { ok: "#22c55e", warning: "#ef4444", unknown: "#94a3b8" };
const SLAB = { ok: "✅ Activo", warning: "⚠️ Problemas", unknown: "ℹ️ Sin datos" };
...
const STATIONS = [
  ...
  { name:"Cargador Municipal", addr:"Ronda Bernat Aliño 57, Sueca", zone:"Sueca", network:"Ayuntamiento", rating:3.0, hours:"24h", status:"ok", notes:"Funciona bien.", conn:"CCS, Type 2", lat:39.2037416, lng:-0.3076841 },
  { name:"Fenide Energía", addr:"Plaça Ajuntament 10, Sueca", zone:"Sueca", network:"Fenide", rating:2.3, hours:"24h", status:"warning", notes:"Averías frecuentes.", conn:"Type 2", lat:39.2026217, lng:-0.3104931 },
  ...
  { name:"Iberdrola | bp pulse", addr:"C/ Periodista Azzati s/n, Port de Sagunt", zone:"Sagunto", network:"Iberdrola", rating:2.8, hours:"24h", status:"warning", notes:"Periodos fuera de servicio.", conn:"Ultra CCS, CHAdeMO", lat:39.668097, lng:-0.225118 },
  { name:"Iberdrola | bp pulse (IGSA) ⭐", addr:"C/ Prof. Beltrán Báguena 4, Valencia", zone:"Valencia", network:"Iberdrola", rating:4.3, hours:"24h", status:"ok", notes:"94 kW. Parking IGSA.", conn:"Ultra CCS, CHAdeMO", lat:39.4804177, lng:-0.3889125 },
  ...
  { name:"Wenea (Ciutat Vella)", addr:"Pl. Sant Agustí s/n, Valencia", zone:"Valencia", network:"Wenea", rating:null, hours:"24h", status:"ok", notes:"Centro histórico.", conn:"CCS, Type 2", lat:39.4686457, lng:-0.3798529 },
  { name:"Iberdrola (Roger de Llòria)", addr:"C/ Roger de Llòria 10, Valencia", zone:"Valencia", network:"Iberdrola", rating:3.0, hours:"24h", status:"ok", notes:"Precio varía según método de pago.", conn:"CCS, Type 2", lat:39.4692034, lng:-0.3746765 },
];
...
function App() {
  ...
  const [aiMsgs,setAiMsgs]= useState([{ role:"assistant", content:"¡Hola Paola! Soy tu asistente para el VW e-up! 2014. Tus datos se guardan en Google Sheets automáticamente. ¿En qué te ayudo?" }]);
  ...
  const saveBateria = async val => {
    ...
    await apiPost("saveConfig", { clave:"bateria_actual", valor:String(val) });
  };
  ...
  const saveDeg = async () => {
    if (!degF.realRange) { showToast("Introduce la autonomía real",false); return; }
    setSyncing(true);
  ...
  const sendAI = async () => {
    ...
    const ld = sohList[0];
    const sys = `Eres experto en VE y asistente del VW e-up! 2014 de Paola. DATOS: Batería 16kWh | Autonomía oficial 133km | SOH:${ld?ld.soh+"%":"no medido"} | Consumo medio:${ac}kWh/100km | Carga actual:${currentCharge}% | Registros:${registros.length} | Km totales:${registros.reduce((s,l)=>s+(+l.km),0)}km | Uso habitual 120km/día L-V | Zona Sueca-Valencia. Responde en español, conciso, máx 3 párrafos.`;
    try {
  ...
  const TABS = [["home","🏠"],["diary","📅 Diario"],["seasons","🌡️"],["battery","🔋"],["map","📍"],["advisor","🤖 IA"],["stats","📊"]];

  const btn  = (label,style={},onClick,disabled=false) => h("button",{style:{...bP,...style},onClick,disabled},label);
  const chip = (label,active,color,onClick) => h("button",{onClick,style:{ padding:"8px 4px", borderRadius:"8px", border:"none", cursor:"pointer", fontSize:"12px", fontWeight:600, flex:1, background:active?color:"#334155", color:active?"#fff":"#94a3b8" }},label);
  ...
  if (loadState==="error") return h("div",{style:{background:"#0f172a",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:"24px"}},
    h(Card,{},
      h("div",{style:{textAlign:"center"}},
        ...
        h("div",{style:{fontWeight:700,fontSize:"16px",marginBottom:"8px",color:"#fca5a5"}},"Sin conexión con Sheets"),
        h("div",{style:{color:"#94a3b8",fontSize:"13px",marginBottom:"20px"}},"No se pudo conectar con Google Sheets."),
        btn("🔄 Reintentar",{},loadData)
      )
    )
  );
  ...
  return h("div",{style:{background:"#0f172a",minHeight:"100vh",color:"#f1f5f9",fontFamily:"system-ui,sans-serif",maxWidth:"480px",margin:"0 auto",paddingBottom:"70px"}},
    ...
    h("div",{style:{padding:"16px"}},
      ...
      tab==="home" && h("div",null,
        h(Card,null,
          h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"10px"}},
            h("span",{style:{color:"#94a3b8",fontSize:"13px"}},"Batería actual"),
        ...
        h("div",{style:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px",marginBottom:"14px"}},
          [["Autonomía real",eR+" km","#22c55e"],["kWh disponibles",aK+" kWh","#f59e0b"],["Consumo medio",aC!=="–"?aC+" kWh/100":"Sin datos","#a78bfa"],["SOH batería",soh?soh+"%":"No medido",soh?sC:"#475569"]].map(([lbl,val,color])=>
        ...
        h("div",{style:{background:currentCharge>=55?"#14532d":"#7f1d1d",border:`1px solid ${currentCharge>=55?"#22c55e":"#ef4444"}`,borderRadius:"12px",padding:"14px",textAlign:"center",marginBottom:"14px"}},
          h("div",{style:{fontWeight:700}},currentCharge>=55?"✅ Batería OK para trayecto diario (120 km)":"⚠️ Batería insuficiente para 120 km"),
          h("div",{style:{color:"#94a3b8",fontSize:"12px",marginTop:"4px"}},`Mínimo 55% · Actual ${currentCharge}%`)
        ),
        registros[0] && h(Card,null,
          h("div",{style:{fontWeight:700,marginBottom:"10px",fontSize:"14px"}},"📅 Último registro — "+registros[0].fecha),
          h("div",{style:{display:"flex"}},
            [["Batería",`${registros[0].bateria_salida}%→${registros[0].bateria_llegada}%`,"#3b82f6"],["Km",registros[0].km,"#22c55e"],["kWh/100",registros[0].consumo,registros[0].consumo>18?"#ef4444":registros[0].consumo>15?"#f59e0b":"#22c55e"]].map(([lbl,val,color])=>
        ...
        h("div",{style:{background:"#14532d",border:"1px solid #22c55e",borderRadius:"12px",padding:"12px",display:"flex",alignItems:"center",gap:"10px"}},
          ...
          h("div",null,
            h("div",{style:{fontWeight:600,fontSize:"13px"}},"Datos guardados automáticamente"),
      ...
      tab==="diary" && h("div",null,
        h(Card,null,
          h("div",{style:{fontWeight:700,marginBottom:"14px"}},editId?"✏️ Editando registro":"📅 Registro del trayecto"),
          h(Lbl,null,"Fecha"), h("input",{style:iS,value:dayF.date,onInput:e=>setDayF(f=>({...f,date:e.target.value}))}),
          h("div",{style:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px",margin:"10px 0"}},
            h("div",null,h(Lbl,null,"🔋 % Batería al salir"),h("input",{style:iS,type:"number",placeholder:"85",value:dayF.startPct,onInput:e=>setDayF(f=>({...f,startPct:e.target.value}))})),
            h("div",null,h(Lbl,null,"🏁 % Batería al llegar"),h("input",{style:iS,type:"number",placeholder:"25",value:dayF.endPct,onInput:e=>setDayF(f=>({...f,endPct:e.target.value}))}))
          ),
          h(Lbl,null,"📍 Kilómetros"), h("input",{style:{...iS,marginBottom:"10px"},type:"number",placeholder:"60 ida / 120 día completo",value:dayF.km,onInput:e=>setDayF(f=>({...f,km:e.target.value}))}),
          h(Lbl,null,"🌡️ Temporada"),
          ...
          h("div",{style:{display:"flex",gap:"8px"}},
            h("button",{style:{...bP,background:syncing?"#334155":"#3b82f6"},onClick:saveDay,disabled:syncing},syncing?"Guardando...":"☁️ Guardar en Sheets"),
            editId && h("button",{onClick:()=>{setEditId(null);setDayF({date:tod(),startPct:"",endPct:"",km:"",mode:"eco",chargeLocation:"none",season:getSeasonByMonth()});},style:{...bP,background:"#334155",width:"auto",padding:"10px 14px"}},"✕")
          )
        ),
        registros.length===0
          ? h("div",{style:{textAlign:"center",color:"#475569",padding:"30px"}},"Sin registros aún. ¡Añade el primero!")
          : h(Card,null,
            h("div",{style:{fontWeight:700,marginBottom:"14px"}},`Historial — ${registros.length} registro${registros.length!==1?"s":""}`),
            registros.map(log=>{
              ...
              return h("div",{key:log.id,style:{borderBottom:"1px solid #334155",paddingBottom:"12px",marginBottom:"12px"}},
                ...
                h("div",{style:{display:"flex"}},
                  [["Batería",`${log.bateria_salida}%→${log.bateria_llegada}%`,"#3b82f6"],["kWh/100",log.consumo,cc],["Carga",loc.label,loc.color]].map(([lbl,val,color])=>
                    h("div",{key:lbl,style:{flex:1,textAlign:"center"}},
                      h("div",{style:{fontWeight:700,color,fontSize:"13px"}},val),
                      h("div",{style:{fontSize:"10px",color:"#64748b"}},lbl)
                    )
        ...
        return h("div",null,
          h(Card,{style:{border:`2px solid ${cur.color}`}},
            h("div",{style:{display:"flex",alignItems:"center",gap:"8px",marginBottom:"10px"}},
              h("span",{style:{fontSize:"24px"}},cur.label.split(" ")[0]),
              h("div",null,
                h("div",{style:{fontWeight:700,fontSize:"15px"}},"Ahora — "+cur.label.split(" ")[1]),
                h("div",{style:{color:"#94a3b8",fontSize:"12px"}},"Autonomía esperada: "+cur.rangeEst)
              )
            ),
            h("div",{style:{background:"#0f172a",borderRadius:"8px",padding:"10px",fontSize:"12px",color:"#94a3b8",marginBottom:"10px"}},"💡 "+cur.tip),
            curSt.logs.length>0
              ? h("div",{style:{display:"flex"}},
                [["Registros",curSt.logs.length,cur.color],["Km",curSt.totalKm+" km",cur.color],["kWh/100",curSt.avgCons,cur.color],["Autonomía",curSt.realRange+" km",cur.color]].map(([lbl,val,color])=>
                  h("div",{key:lbl,style:{flex:1,textAlign:"center"}},
              ...
              : h("div",{style:{color:"#475569",fontSize:"12px",textAlign:"center"}},"Sin registros esta temporada aún.")
          ),
          h(Card,null,
            ...
            stats.map(s=>h("div",{key:s.id,style:{marginBottom:"14px"}},
              h("div",{style:{display:"flex",justifyContent:"space-between",fontSize:"13px",marginBottom:"4px"}},
                ...
                s.avgCons
                  ? h("span",{style:{color:"#94a3b8",fontSize:"11px"}},`${s.avgCons} kWh/100 · ${s.logs.length} reg. · ${s.realRange} km`)
          ...
          h(Card,null,
            h("div",{style:{fontWeight:700,marginBottom:"12px"}},"📋 Tabla resumen"),
            h("table",{style:{width:"100%",borderCollapse:"collapse",fontSize:"12px"}},
              h("thead",null,h("tr",{style:{borderBottom:"1px solid #334155"}},
                ["Temporada","Reg.","Km","kWh/100","Autonomía"].map(col=>h("th",{key:col,style:{padding:"6px 4px",color:"#94a3b8",fontWeight:600,textAlign:"left"}},col))
              )),
              h("tbody",null,stats.map(s=>h("tr",{key:s.id,style:{borderBottom:"1px solid #1e293b"}},
                h("td",{style:{padding:"8px 4px",color:s.color,fontWeight:600}},s.label),
                h("td",{style:{padding:"8px 4px"}},s.logs.length||"–"),
                h("td",{style:{padding:"8px 4px"}},s.totalKm||"–"),
                h("td",{style:{padding:"8px 4px",fontWeight:700,color:s.avgCons?s.color:"#475569"}},s.avgCons||"–"),
                h("td",{style:{padding:"8px 4px",fontWeight:700,color:s.realRange?s.color:"#475569"}},s.realRange?s.realRange+" km":"–")
              )))
            )
          )
        );
      ...
      tab==="battery" && h("div",null,
        h(Card,null,
          h("div",{style:{fontWeight:700,marginBottom:"6px"}},"🔋 Salud de la batería (SOH)"),
          h("div",{style:{color:"#94a3b8",fontSize:"12px",marginBottom:"14px"}},"Carga al 100% y anota cuántos km reales alcanzas hasta llegar a 0%."),
          soh
            ? h("div",null,
              h("div",{style:{textAlign:"center",marginBottom:"16px"}},
                ...
                h("div",{style:{marginTop:"8px",background:"#0f172a",borderRadius:"8px",padding:"8px",fontSize:"12px"}},
                  soh>=85?"✅ Batería en buen estado":soh>=70?"⚠️ Degradación moderada.":"❌ Degradación severa. Consulta taller."
                )
              ),
              ...
              h("div",{style:{display:"flex",justifyContent:"space-between",fontSize:"10px",color:"#475569"}},
                h("span",null,"Crítico <70%"),h("span",null,"Revisar 70–85%"),h("span",null,"OK >85%")
              )
            )
            : h("div",{style:{textAlign:"center",padding:"16px",color:"#475569",fontSize:"13px"}},"Sin mediciones. Registra la primera abajo.")
        ),
        h(Card,null,
          h("div",{style:{fontWeight:700,marginBottom:"14px"}},"📏 Nueva medición SOH"),
          h(Lbl,null,"Autonomía real (km de 100% a 0%)"), h("input",{style:{...iS,marginBottom:"10px"},type:"number",placeholder:"Ej: 105",value:degF.realRange,onInput:e=>setDegF(f=>({...f,realRange:e.target.value}))}),
          h(Lbl,null,"Fecha"), h("input",{style:{...iS,marginBottom:"10px"},value:degF.date,onInput:e=>setDegF(f=>({...f,date:e.target.value}))}),
          h(Lbl,null,"Condiciones"), h("input",{style:{...iS,marginBottom:"12px"},placeholder:"Ej: primavera, autovía",value:degF.notes,onInput:e=>setDegF(f=>({...f,notes:e.target.value}))}),
          ...
          h("button",{style:{...bP,background:syncing?"#334155":"#3b82f6"},onClick:saveDeg,disabled:syncing},syncing?"Guardando...":"☁️ Guardar medición")
        ),
      ...
      tab==="advisor" && h("div",{style:{display:"flex",flexDirection:"column",height:"calc(100vh - 210px)"}},
        ...
        h("div",{style:{display:"flex",gap:"6px",flexWrap:"wrap",marginBottom:"10px"}},
          ["¿Cuánta autonomía me queda?","Consejos para ahorrar batería","¿Cuándo debo recargar?","¿Cómo va mi degradación?"].map(q=>
        ...
        h("div",{style:{display:"flex",gap:"8px"}},
          h("input",{style:{...iS,flex:1},placeholder:"Pregunta sobre tu vehículo…",value:aiIn,onInput:e=>setAiIn(e.target.value),onKeyDown:e=>e.key==="Enter"&&sendAI()}),
      ...
      tab==="stats" && h("div",null,
        ...
        h(Card,null,
          h("div",{style:{fontWeight:700,marginBottom:"12px"}},"Modos de conducción"),
          MODES.map(m=>{
